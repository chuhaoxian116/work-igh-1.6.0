...
void ConfigureCurrentThreadRealtime() {
  ...
  if (result != 0) {
    std::fprintf(stderr,
                 "warning: pthread_setschedparam(SCHED_FIFO) failed: %d\n",
                 result);
  }
}
...
void RunCycleThread(orchestrator::RobotEthercatOrchestrator &application,
                    uint32_t cycle_time_ns, std::atomic_bool &keep_running,
  ...
  while (keep_running.load()) {
    const int sleep_result = clock_nanosleep(
        CLOCK_MONOTONIC, TIMER_ABSTIME, &wakeup_time, nullptr);
...
int main() {
  ...
  std::thread cycle_thread(RunCycleThread, std::ref(application),
