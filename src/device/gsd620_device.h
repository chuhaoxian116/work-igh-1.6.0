...
public:
   ...
   */
  const Gsd620CyclicData &cyclic_data() const {
      return cyclic_data_;
  }
   ...
   */
  const Gsd620Configuration &configuration() const {
...
private:
    ...
    std::array<ec_pdo_entry_reg_t, kPdoEntryCount + 1> pdo_entry_regs_{}; // 含结束标记的 IgH PDO entry 注册表。
